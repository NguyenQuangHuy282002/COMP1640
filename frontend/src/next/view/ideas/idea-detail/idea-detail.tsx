...
function IdeaDetail() {
  ...
  useEffect(() => {
    const getIdea = async () => await Http.get(`/api/v1/idea/detail?id=${id}`).then(res => {
      ...
      data ? (
        <Layout className="layout"
          ...
          <StyledContent>
            ...
            {
            data[0] ?. files.length > 0 && <FileDisplay files={
              data[0] ?. files
            }></FileDisplay>
          }
          ...
          {
          new Date(data[0] ?. specialEvent.finalCloseDate) > new Date() ? <StyledContent>
            <Space style={
                {
                  padding: '10px 24px',
                  width: '100%'
              ...
              <CreateComment user={
                ...
                ideaId={id}
                email={
                  data[0] ?. publisherId ?. email
                }/>
            </Space>
          </StyledContent> : <StyledContent style={{textAlign:'center'}}>
           <Typography.Text type="danger" style={{ fontSize: "18px", fontFamily: "Palatino Linotype" }}>This Idea Has Reach The Final Closure Date, You Are Not Allow To Comment Anymore</Typography.Text>
          </StyledContent>
        }
          <StyledContent>
            <div style={
              ...
              {
              showComment ? <CommentsList id={id}
                updateIdea={updateIdea}></CommentsList> : <></>
            } </div>
          </StyledContent>
        </Layout>
      ) : (
