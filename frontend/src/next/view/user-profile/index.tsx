...
function UserProfile() {
  ...
  useEffect(() => {
    const getMyIdea = async () => {
      await Http.get('/api/v1/idea/ideasOfUser?uid=me')
        .then(res => setMyIdea(res.data.data))
    ...
    <Row gutter={{ xs: 8, sm: 16, md: 24 }} style={{ padding: 20 }}>
      <Col className="gutter-row" xs={24} sm={24} md={6} xxl={6}>
        ...
        >
          <Space direction="vertical" size={16}>
            ...
            <Text>
              <b>DOB: </b>
              {state?.birthday ? formatDayTime(state.birthday) : null}
            </Text>
            <Text>
              <b>Phone number: </b>
              {state?.phone}
      ...
      <Col className="gutter-row" xs={24} sm={24} md={18} xxl={18}>
        <Card tabList={tabList} activeTabKey={activeTabKey} onTabChange={setActiveTabKey}>
          {activeTabKey === 'edit' ? (
            <EditProfileForm userInfo={state} />
          ) : activeTabKey === 'about' ? (
            <UserInfomation />
          ) : (
            <ListIdeas ideas={myIdea} />
          )}
        </Card>
      </Col>
    </Row>
